...
def start_keyboard() -> ReplyKeyboardMarkup:
    ...
    first_key = KeyboardButton(text=key_text.SIGN_UP)
    second_key = KeyboardButton(text=key_text.BUG_REPORT)
    third_key = KeyboardButton(text=key_text.HELP)
...
def admin_menu() -> ReplyKeyboardMarkup:
    """
    Функция - создаёт и возвращает клавиатуру админ-панели
    :return: ReplyKeyboardMarkup
    """
    keyboards = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    first_key = KeyboardButton(text=key_text.OPEN_ZBT)
    second_key = KeyboardButton(text=key_text.CLOSE_ZBT)
    third_key = KeyboardButton(text=key_text.SEND_MESSAGE)
    return keyboards.add(first_key, second_key, third_key)
